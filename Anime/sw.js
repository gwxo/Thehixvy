...
const CACHE_NAME = 'prananime'; // Bumped version to force update

// Update these paths to point specifically to your movie folder assets
const CORE_ASSETS = [
  '/Anime/',              // The movie app entry point
  '/Anime/index.html',    // Explicitly cache the movie's HTML file
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(
      fetch(event.request).catch(() => {
        ...
        return caches.match('/movie/') || caches.match('/Anime/index.html');
      })
    );
  } else {
